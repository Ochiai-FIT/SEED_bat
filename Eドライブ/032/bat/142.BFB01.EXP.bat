...
SET OUTFILENAME=0142
SET EXPFILE=%~DP0../exp/%OUTFILENAME%
SET HULFTID=VFBUO043

REM ===== HULFT SEND DELETE ===============
SET HULFT=D:\HULFTFILES\SEND\%OUTFILENAME%
DEL /Q %HULFT%\*

SET MSG=%DATE% %TIME%  →  EXPORT処理(定量特売ファイル　（事前打出）)
ECHO %MSG%
>> %OUTFILE% ECHO %MSG%
"%MYSQLPATH%mysql" %OPTION% -h %HOST% %DB_NAME% -u %USER_ID% -N < %~dp0142.BFB01.EXP.sql >%EXPFILE% 2>>%OUTFILE%
