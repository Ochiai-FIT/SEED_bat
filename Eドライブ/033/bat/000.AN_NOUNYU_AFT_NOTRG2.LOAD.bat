...
SET MSG=%DATE% %TIME% INAIF.AN_NOUNYU_AFT ロード処理（全特アンケート有 納入データ(反映後)）:反映対象外データ2(事前発注)
ECHO %MSG%
>> %OUTFILE% ECHO %MSG%
...
"%MYSQLPATH%mysql" %OPTION% -h %HOST% %DB_NAME% -u %USER_ID% --show-warnings -vv -e "ANALYZE TABLE INAIF.AN_NOUNYU_AFT;" >>%OUTFILE% 2>&1
IF NOT %ERRORLEVEL%==0 GOTO ERROR
