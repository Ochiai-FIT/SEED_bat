...
SET MSG=%DATE% %TIME%  → INAIF.JH_TEN_SYUSEI ロード処理（店舗修正データ(本締め)作成）
ECHO %MSG%
>> %OUTFILE% ECHO %MSG%
REM BINKBN追加。ROW_NUMBER() OVER (PARTITION BY BMNCD, TENCD, SHNCD, NNDT, BINKBN ORDER BY HTSU DESC) AS RNO
...
"%MYSQLPATH%mysql" %OPTION% -h %HOST% %DB_NAME% -u %USER_ID% --show-warnings -vv -e "ANALYZE TABLE INAIF.JH_TEN_SYUSEI;" >>%OUTFILE% 2>&1
IF NOT %ERRORLEVEL%==0 GOTO ERROR
