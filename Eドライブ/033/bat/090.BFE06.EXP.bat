...
SET OUTFILENAME=0090
SET EXPFILE=%~DP0../exp/%OUTFILENAME%
SET HULFTID=VFBUO001

REM ===== HULFT SEND DELETE ===============
SET HULFT=D:\HULFTFILES\SEND\%OUTFILENAME%
DEL /Q %HULFT%\*

SET MSG=%DATE% %TIME% →  EXPORT処理(全特アンケート有店舗)
ECHO %MSG%
>> %OUTFILE% ECHO %MSG%
"%MYSQLPATH%mysql" %OPTION% -h %HOST% %DB_NAME% -u %USER_ID% -N <  %~dp0090.BFE06.EXP.sql >%EXPFILE% 2>>%OUTFILE%
