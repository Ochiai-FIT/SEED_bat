...
SET OUTFILENAME=0092
SET EXPFILE=%~DP0../exp/%OUTFILENAME%
SET HULFTID=VFBUO003

REM ===== HULFT SEND DELETE ===============
SET HULFT=D:\HULFTFILES\SEND\%OUTFILENAME%
DEL /Q %HULFT%\*

SET MSG=%DATE% %TIME% →  EXPORT処理(全特アンケート無店舗)
ECHO %MSG%
>> %OUTFILE% ECHO %MSG%
"%MYSQLPATH%mysql" %OPTION% -h %HOST% %DB_NAME% -u %USER_ID% -N <  %~dp0092.BFE06.EXP.sql >%EXPFILE% 2>>%OUTFILE%
