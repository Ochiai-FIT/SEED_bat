...
REM ===== HULFT REV MOVE =================
REM DEL /Q %~dp0\DATA\*
ROBOCOPY  D:\HULFTFILES\RCV\0074 %~dp0DATA /R:0 /E /NFL /NP /XD backup bkup RcvTemp /MOV >> %OUTFILE%
...
rem =====================================
rem 074 削除可ファイルロード処理
rem =====================================
set callbat=bat\074.AASTF.LOAD.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error

rem =====================================
rem 074 商品コードチェックデータロード処理
rem =====================================
set callbat=bat\074.A11SCD.LOAD.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error
...
rem 074 商品コード削除処理
rem =====================================
set callbat=bat\074.MSTSHN.DELETE.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error

rem =====================================
rem 076 商品マスタ削除データエクスポート処理
rem =====================================
set callbat=bat\076.AASTF.EXP.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error

rem =====================================
rem 652 完了通知(年次) 商品マスタ削除
rem =====================================
set callbat=bat\652.完了通知.EXP.BAT
...
echo !RESULT!
:SUCCESS
echo %DATE% %TIME% 不定期処理(103)が正常に終了しました >> %outfile%
