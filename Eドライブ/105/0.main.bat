...
REM ===== HULFT REV MOVE =================
DEL /Q %~dp0\DATA\*
ROBOCOPY  D:\HULFTFILES\RCV\0075 %~dp0DATA /R:0 /E /NFL /NP /XD backup bkup RcvTemp /MOV >> %OUTFILE%
...
rem =====================================
rem 075 商品マスタ一括変更データロード処理
rem =====================================
set callbat=bat\075.A10MNT.LOAD.bat
...
rem 075 商品マスタマージ処理
rem =====================================
set callbat=bat\075.MSTSHN.MRG.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error

rem =====================================
rem 075 商品マスタ_予約マージ処理
rem =====================================
set callbat=bat\075.MSTSHN_Y.MRG.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error

rem =====================================
rem 075 商品マスタ変換テーブル(エラー)
rem =====================================
set callbat=bat\075.ERR.EXP.bat
echo %callbat% >> %outfile%
call %~dp0%callbat%
if errorlevel 1 goto error
